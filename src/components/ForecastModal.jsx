...
import { getAiAdvice } from '../services/aiService'; // Re-use your existing service
...
const ForecastModal = ({ onClose }) => {
  ...
  const handlePredict = async () => {
    if (!desc) return;
    setLoading(true);
    // Reuse your AI service!
    const advice = await getAiAdvice(null, desc);
    setResult(advice);
    setLoading(false);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
      ...
      >
        ...
        <div className="p-8 border-b border-white/10 bg-gradient-to-r from-indigo-900/50 to-purple-900/50">
          ...
          {/* Updated description to match the hackathon theme */}
          ...
          {!result && (
            <div className="space-y-4">
              ...
              >
                ...
                {loading ? "Calculating Probability..." : "Run Prediction"}
              </button>
            </div>
          )}

          {/* Result Section (The "Show There" part) */}
          {result && (
            <div className="animate-in fade-in slide-in-from-bottom-4">
               <div className="grid grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="bg-purple-500/10 border border-purple-500/30 p-6 rounded-2xl text-center">
                    ...
                    <p className="text-lg font-bold text-white mt-2 leading-tight">{result.impact || "Medium"}</p>
                  </div>
               </div>

               <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
                  <h4 className="font-bold text-slate-300 mb-2">AI Analysis:</h4>
                  <p className="text-slate-400 text-sm">{result.precautions[0]}</p>
               </div>

               <button 
                 onClick={() => setResult(null)}
                 className="mt-6 w-full py-3 text-slate-400 hover:text-white font-semibold"
               >
